...
def test_public_projects_are_tenant_aware(client):
...
def test_login_returns_signed_token_and_identity_is_scoped(client):
    token = login(client, "student@demo.edu", "DemoStudent123!")
    response = client.get(
        "/api/v1/identity/me",
...
def test_student_cannot_write_attendance(client):
    token = login(client, "student@demo.edu", "DemoStudent123!")
    response = client.post(
...
def test_student_ai_gate_is_authorized_but_provider_can_be_disabled(client):
    token = login(client, "student@demo.edu", "DemoStudent123!")
    response = client.post(
        "/api/v1/ai/execute",
        headers={"Authorization": f"Bearer {token}"},
...
def test_president_can_create_group(client):
    token = login(client, "president@demo.edu", "DemoPresident123!")
    response = client.post(
