    ...
    document.querySelector('#project-list').innerHTML = projects.length
      ? projects.map((project) => `
          <article>
            ...
            <small>${escapeHtml(project.department || '')}</small>
          </article>
        `).join('')
      : '<article><b>لا توجد مشاريع منشورة بعد.</b><p>ستظهر هنا المشاريع التي تعتمدها الجامعة.</p></article>';
  } catch (error) {
    document.querySelector('#project-list').innerHTML =
      '<article><b>تعذر تحميل المشاريع.</b><p>تحقق من اتصال الخادم أو إعدادات الجامعة.</p></article>';
  }
}
...
document.querySelector('#login-form')?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      status.textContent = 'تعذر تسجيل الدخول: ' + (data.detail || 'تحقق من البيانات');
      return;
    }
